...
- Les options `failover_mode` et `seeding_mode` ne sont pas prises en charge quand `availability_mode` est défini sur `configuration_only` pour un réplica. Vous trouverez un exemple [ici](../../linux/sql-server-linux-availability-group-configure-ha.md).
