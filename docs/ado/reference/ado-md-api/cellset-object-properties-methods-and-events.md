---
description: Propriétés, méthodes et événements de l’objet Cellset
title: Propriétés, méthodes et événements de l’objet CellSet | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: ado
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: reference
helpviewer_keywords:
- Cellset object [ADO MD], members
ms.assetid: fb303e33-5a85-4e4e-81db-acaaa6f53799
author: rothja
ms.author: jroth
ms.openlocfilehash: ffc1a3e6a96a3910d02bf7d94b2a655cad5b1381
ms.sourcegitcommit: 33f0f190f962059826e002be165a2bef4f9e350c
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/30/2021
ms.locfileid: "99169941"
---
# <a name="cellset-object-properties-methods-and-events"></a>Propriétés, méthodes et événements de l’objet Cellset
## <a name="propertiescollections"></a>Propriétés/collections  
 [Propriété ActiveConnection](./activeconnection-property-ado-md.md)  
  
 [Axes, collection](./axes-collection-ado-md.md)  
  
 [FilterAxis, propriété](./filteraxis-property-ado-md.md)  
  
 [Item, propriété (CellSet)](./item-property-ado-md-cellset.md)  
  
 [Collection Properties](../ado-api/properties-collection-ado.md)  
  
 [Propriété source](./source-property-ado-md.md)  
  
 [Propriété State](./state-property-ado-md.md)  
  
## <a name="methods"></a>Méthodes  
 [Close, méthode](./close-method-ado-md.md)  
  
 [Méthode Open](./open-method-ado-md.md)  
  
## <a name="events"></a>Événements  
 Aucun.  
  
## <a name="see-also"></a>Voir aussi  
 [Cellset, objet (ADO MD)](./cellset-object-ado-md.md)